...
struct AuthView: View {
    ...
    var authFieldsUI: some View {
        VStack(spacing: 15) {
            ...
            Button {
                ...
                Text(isSignUp ? "Already have an account? Log In" : "Don't have an account? Sign Up").font(.footnote)
            }
            
            // MARK: - MANDATORY EULA LINK FOR APPLE REVIEW
            if isSignUp {
                Link("By signing up, you agree to our Terms of Use and EULA.", destination: URL(string: "https://docs.google.com/document/d/1Xoxju0dHO8gWIALPOV0z0dnVKFQe9__qMW7CNeoXwno/")!)
                    .font(.caption2)
    ...
    var confirmationUI: some View {
        VStack(spacing: 25) {
            Image(systemName: "envelope.circle.fill").font(.system(size: 70)).foregroundColor(.blue)
            VStack(spacing: 10) {
            ...
            VStack(spacing: 15) {
                Button { Task { await authViewModel.resendConfirmationEmail(email: email) } } label: {
                    HStack {
                ...
                Button("Back to Login") { authViewModel.showConfirmationMessage = false; isSignUp = false }
                    .font(.callout).foregroundColor(.secondary)
            }
        }.padding().background(Color(.systemGray6)).cornerRadius(20)
    }
}
